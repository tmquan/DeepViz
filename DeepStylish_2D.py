	...
	def vgg19_decoder(self, inputs, name='VGG19_Decoder'):
		with argscope([Conv2D], kernel_shape=3, nl=BNLReLU):	
			with argscope([Deconv2D], kernel_shape=3, strides=(2,2), nl=BNLReLU):
				pool3 = Subpix2D('pool3',   inputs,  NB_FILTERS*4)  # 16
				...
				pool1 = Subpix2D('pool1',   conv2_1, NB_FILTERS*1)  # 64
		...
		with tf.variable_scope('gen'):
			with tf.device('/device:GPU:0'):
				...
				with tf.variable_scope('encoder_vgg19'):
					...
					list_vol3d_encoded = tf.split(vol3d_encoded, num_or_size_splits=16, axis=0)
			...
			with tf.device('/device:GPU:1'):
				...
				with tf.variable_scope('decoder_vgg19'):
					...
					list_chose_encoded = tf.split(chose_encoded, num_or_size_splits=16, axis=0)
					list_vol3d_decoded = tf.split(vol3d_decoded, num_or_size_splits=16, axis=0)

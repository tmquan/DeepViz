...
def Conv3D(
        ...
        activation=None,
        use_bias=False,
        kernel_initializer=tf.contrib.layers.xavier_initializer(), #tf.initializers.variance_scaling(distribution='uniform'),
...
class Model(ModelDesc):
	...
	def vol3d_encoder(self, x, name='Vol3D_Encoder'):
		with argscope([Conv3D], kernel_shape=4, padding='SAME', nl=tf.nn.elu):
			...
			"""
			inputs_ = x
			conv1 = tf.layers.conv3d(inputs=inputs_, filters=4, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='conv1')
			pool1 = tf.layers.max_pooling3d(conv1, pool_size=(2,2,2), strides=(2,2,2), padding='SAME')
			conv2 = tf.layers.conv3d(inputs=pool1, filters=8, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='conv2')
			pool2 = tf.layers.max_pooling3d(conv2, pool_size=(2,2,2), strides=(2,2,2), padding='SAME')
			conv3 = tf.layers.conv3d(inputs=pool2, filters=16, kernel_size=(3,3,3), padding='SAME' , strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='conv3')
			pool3 = tf.layers.max_pooling3d(conv3, pool_size=(2,2,2), strides=(2,2,2), padding='SAME')
			conv4 = tf.layers.conv3d(inputs=pool3, filters=32, kernel_size=(3,3,3), padding='SAME' , strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='conv4')
			pool4 = tf.layers.max_pooling3d(conv4, pool_size=(2,2,2), strides=(2,2,2), padding='SAME')
			conv5 = tf.layers.conv3d(inputs=pool4, filters=64, kernel_size=(3,3,3), padding='SAME' , strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='conv5')
			pool5 = tf.layers.max_pooling3d(conv5, pool_size=(2,2,2), strides=(2,2,2), padding='SAME')
			conv6 = tf.layers.conv3d(inputs=pool5, filters=1024, kernel_size=(3,3,3), padding='SAME' , strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='conv6')
			pool6 = tf.layers.max_pooling3d(conv6, pool_size=(2,2,2), strides=(2,2,2), padding='SAME')
	...
	def vol3d_decoder(self, x, name='Vol3D_Decoder'):
		with argscope([Conv3DTranspose], kernel_shape=4, padding='SAME', nl=tf.nn.elu):
			...
			x = (LinearWrap(x)
				...
				.Conv3DTranspose('conv1a',   1, strides = 2, padding='SAME', use_bias=True) #
				()) 
			"""
			#decoder
			inputs_ = x
			print(inputs_.get_shape().as_list())
			unpool1 = K.resize_volumes(inputs_,2,2,2,"channels_last")
			print(unpool1.get_shape().as_list())
			deconv1 = tf.layers.conv3d_transpose(inputs=unpool1, filters=64, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='unpool1')
			print(deconv1.get_shape().as_list())
			unpool2 = K.resize_volumes(deconv1,2,2,2,"channels_last")
			print(unpool2.get_shape().as_list())
			deconv2 = tf.layers.conv3d_transpose(inputs=unpool2, filters=32, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='unpool2')
			unpool3 = K.resize_volumes(deconv2,2,2,2,"channels_last")
			deconv3 = tf.layers.conv3d_transpose(inputs=unpool3, filters=16, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='unpool3')
			unpool4 = K.resize_volumes(deconv3,2,2,2,"channels_last")
			deconv4 = tf.layers.conv3d_transpose(inputs=unpool4, filters=8, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='unpool4')
			unpool5 = K.resize_volumes(deconv4,2,2,2,"channels_last")
			deconv5 = tf.layers.conv3d_transpose(inputs=unpool5, filters=4, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='unpool5')
			unpool6 = K.resize_volumes(deconv5,2,2,2,"channels_last")
			deconv6 = tf.layers.conv3d_transpose(inputs=unpool6, filters=1, kernel_size=(3,3,3), padding='SAME', strides=[1,1,1], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, name='unpool6')
			# unpool7 = K.resize_volumes(deconv6,2,2,2,"channels_last")
			
			# print(unpool1.get_shape().as_list())
			# print(deconv1.get_shape().as_list())
			# print(unpool2.get_shape().as_list())
			# print(deconv2.get_shape().as_list())
			# print(unpool3.get_shape().as_list())
			# print(deconv3.get_shape().as_list())
			# print(unpool4.get_shape().as_list())
			# print(deconv4.get_shape().as_list())
			# print(unpool5.get_shape().as_list())
			# print(deconv5.get_shape().as_list())
			# print(unpool6.get_shape().as_list())
		...
		# 		x = (LinearWrap(x)
		# 			.tf_space_to_batch('bs1')
		# 			.Subpix2D('conv1', 3)
		# 			.tf_space_to_batch('bs2')
		# 			.Subpix2D('conv2', 3)
		# 			.tf_space_to_batch('bs3')
		# 			.Subpix2D('conv3', 3)
		# 			.tf_space_to_batch('bs4')
		# 			.Subpix2D('conv4', 3)
		# 			())
		# 		return x
	...
	def _build_graph(self, inputs):
		...
		self.cost = tf.reduce_sum(losses, name='self.cost')+wd_g
...
class RenderingRunner(Callback):
	...
	def _trigger(self):
		...
		resultDir = 'result_stylish_3d_vgg_keras/'

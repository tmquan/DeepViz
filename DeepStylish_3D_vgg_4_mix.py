...
def normalize(v):
	...
	return v / tf.reduce_mean(v, axis=[1], keepdims=True)
###################################################################################################
...
class Model(ModelDesc):
	def _build_adain_layers(self, content, style, eps=1e-6, name='adain', is_normalized=True):
		with tf.variable_scope(name):
			...
			s_mean, s_var = tf.nn.moments(style,   axes=[1], keep_dims=True)
	...
	def vol3d_encoder(self, x, name='Vol3D_Encoder'):
		with argscope([Conv3D], kernel_shape=4, padding='SAME', nl=tf.nn.elu):
			...
			x = (LinearWrap(x)
				.Conv3D('conv1a',   16, strides = 2, padding='SAME') # 1 128 128 128 16
				.Conv3D('conv2a',   32, strides = 2, padding='SAME') # 1  64  64  64 32
				.Conv3D('conv3a',   64, strides = 2, padding='SAME') # 1  32  32  32 64
				.Conv3D('conv4a',  128, strides = 2, padding='SAME') # 1  16  16  16 128
				.Conv3D('conv5a',  256, strides = 2, padding='SAME') # 1   8   8   8 256
				.Conv3D('conv6a',  512, strides = 2, padding='SAME') # 1   4   4   4 512 = 32768
				()) 
			fc6   = FullyConnected('fc6', x ,	4096, activation=tf.nn.elu)
			fc7   = FullyConnected('fc7', fc6,  4096, activation=tf.nn.relu)
			return fc7
		
	@auto_reuse_variable_scope
	def vol3d_decoder(self, x, name='Vol3D_Decoder'):
		with argscope([Conv3DTranspose], kernel_shape=4, padding='SAME', nl=tf.nn.relu):
			x = FullyConnected('fc6', x,   4096, activation=tf.nn.relu)
			x = FullyConnected('fc7', x,  32768, activation=tf.nn.relu)
			...
			x = (LinearWrap(x)
				...
				.Conv3DTranspose('conv1a',   3, strides = 2, padding='SAME') #
				()) 
			x = tf.squeeze(x)
			x = x + VGG19_MEAN_TENSOR
			return x
	...
	def vgg19_encoder(self, inputs, name='VGG19_Encoder'):
		with varreplace.freeze_variables():
			with argscope([Conv2D], kernel_shape=3, nl=tf.nn.relu):
				...
				fc6   = FullyConnected('fc6', pool5,4096, activation=tf.nn.relu)
				fc7   = FullyConnected('fc7', fc6,  4096, activation=tf.nn.relu)
				return fc7
	...
	def vgg19_decoder(self, inputs, name='VGG19_Decoder'):
		...
		with argscope([Conv2D], kernel_shape=3, nl=BNLReLU):	
			with argscope([Deconv2D], kernel_shape=3, strides=(2,2), nl=BNLReLU):
				fc7 = FullyConnected('fc7', inputs,  4096, activation=tf.nn.relu)
				fc6 = FullyConnected('fc6', fc7,    32768, activation=tf.nn.relu)
				
				# pool5 = Subpix2D('pool5',   inputs,  512)  # 16
				# conv5_4 = Conv2D('conv5_4', pool5,   512)
				# conv5_3 = Conv2D('conv5_3', conv5_4, 512)
				# conv5_2 = Conv2D('conv5_2', conv5_3, 512)
				# conv5_1 = Conv2D('conv5_1', conv5_2, 512)
				conv5_1 = tf.reshape(fc6, [-1, 8, 8, 512], name='conv5_1')

				pool4 = Subpix2D('pool4',   conv5_1, 512)  # 16
				...
				conv1_0 = Subpix2D('conv1_0', conv1_1, 3)
				
				results = conv1_0 + VGG19_MEAN_TENSOR
				results = tf.image.resize_images(results, [256, 256])
				return results # List of feature maps
	...
	def _build_graph(self, inputs):
		...
		with tf.variable_scope('gen'):
			...
			with tf.variable_scope('decoder_3d'):
				vol3d_decoded = self.vol3d_decoder(chose_encoded)
		...
		with tf.name_scope('visualization'):
			...
			viz = tf.concat([tf.concat([viz_vol_1, viz_vol_2, viz_vol_3,		viz_img_4], 2), 
							 tf.concat([viz_vol_5, viz_vol_6, viz_vol_7,		viz_img_5], 2), 
							 ], 1)
...
class RenderingRunner(Callback):
	...
	def _trigger(self):
		...
		resultDir = 'result_stylish_3d_vgg_4_mix/'

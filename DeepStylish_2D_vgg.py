	...
	def vgg19_encoder(self, inputs, name='VGG19_Encoder'):
		with varreplace.freeze_variables():
			with argscope([Conv2D], kernel_shape=3, nl=tf.nn.relu):
				...
				pool3 = MaxPooling('pool3', conv3_4, 2)  # 128
	...
	def vgg19_decoder(self, inputs, name='VGG19_Decoder'):
		with argscope([Conv2D], kernel_shape=3, nl=BNLReLU):	
			with argscope([Deconv2D], kernel_shape=3, strides=(2,2), nl=BNLReLU):
				assert inputs.get_shape()[0] == 1
				pool3 = Subpix2D('pool3',   inputs,  64)  # 64
				conv3_4 = Conv2D('conv3_4', pool3,   64)
				conv3_3 = Conv2D('conv3_3', conv3_4, 64)
				conv3_2 = Conv2D('conv3_2', conv3_3, 64)
				conv3_1 = Conv2D('conv3_1', conv3_2, 64)
				pool2 = Subpix2D('pool2',   conv3_1, 32)  # 64
				conv2_2 = Conv2D('conv2_2', pool2, 	 32)
				conv2_1 = Conv2D('conv2_1', conv2_2, 32)
				pool1 = Subpix2D('pool1',   conv2_1, 16)  # 64
				conv1_2 = Conv2D('conv1_2', pool1, 	 16)
				conv1_1 = Conv2D('conv1_1', conv1_2, 16)
		...
		with tf.variable_scope('gen'):
			...
			with tf.variable_scope('encoder_vgg19'):
				with tf.device('/device:GPU:0'):
					list_vol3d = tf.split(vol3d, num_or_size_splits=16, axis=0)
					list_vol3d_encoded = [] 
					for elem_vol3d in list_vol3d:
						elem_vol3d_encoded = self.vgg19_encoder(elem_vol3d)
						list_vol3d_encoded.append(elem_vol3d_encoded)
					vol3d_encoded = tf.concat(list_vol3d_encoded, axis=0)
					
					img2d_encoded  = self.vgg19_encoder(img2d)
					#print(vol3d.get_shape().as_list()[0])
					#print(img2d_encoded.get_shape().as_list()[1::])
					# vol3d_encoded  = self.vgg19_encoder(vol3d)
					"""
					vol3d_encoded = tf.get_variable(dtype=tf.float32, 
												   shape=[vol3d.get_shape().as_list()[0], 32, 32, 512],
												   trainable=False,  
												   name='vol3d_encoded')
					
					list_vol3d = tf.split(vol3d, num_or_size_splits=256, axis=0)
					list_vol3d_encoded = tf.split(vol3d_encoded, num_or_size_splits=256, axis=0)
					for elem_vol3d, elem_vol3d_encoded in zip(list_vol3d, list_vol3d_encoded):
						elem_vol3d_encoded = self.vgg19_encoder(elem_vol3d)
					vol3d_encoded = tf.concat(list_vol3d_encoded, axis=0)
					"""
					# list_vol3d = tf.split(vol3d, num_or_size_splits=256, axis=0)
					# def func_vgg19_encoder(elem_vol3d):
					# 	elem_vol3d = tf.expand_dims(elem_vol3d, axis=0)
					# 	return self.vgg19_encoder(elem_vol3d)
					# list_vol3d_encoded = tf.map_fn(func_vgg19_encoder, list_vol3d)
					# vol3d_encoded = tf.concat(list_vol3d_encoded, axis=0)
			...
			with tf.variable_scope('decoder_vgg19'):
				...
				# Step 3: Run thru the decoder to get the paint image
					...
					"""
					vol3d_decoded = tf.get_variable(dtype=tf.float32, 
												   shape=[vol3d.get_shape().as_list()[0], 256, 256, 3],
												   trainable=False,  
												   name='vol3d_decoded')
					
					list_chose_encoded = tf.split(chose_encoded, num_or_size_splits=256, axis=0)
					list_vol3d_decoded = tf.split(vol3d_decoded, num_or_size_splits=256, axis=0)
					for elem_chose_encoded, elem_vol3d_decoded in zip(list_chose_encoded, list_vol3d_decoded):
						elem_vol3d_decoded = self.vgg19_decoder(elem_chose_encoded)
					vol3d_decoded = tf.concat(list_vol3d_decoded, axis=0)
					"""
				...
				#with tf.variable_scope('decoder_vgg19'):
					list_chose_encoded = tf.split(chose_encoded, num_or_size_splits=256, axis=0)
					list_vol3d_decoded = [] 
					for elem_chose_encoded in list_chose_encoded:
						elem_vol3d_decoded = self.vgg19_decoder(elem_chose_encoded)
						list_vol3d_decoded.append(elem_vol3d_decoded)
					vol3d_decoded = tf.concat(list_vol3d_decoded, axis=0)
...
class RenderingRunner(Callback):
	...
	def _trigger(self):
		...
		resultDir = 'result_stylish_2d_vgg/'

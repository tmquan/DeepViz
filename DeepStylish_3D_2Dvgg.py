	...
	def vol3d_decoder(self, x, name='Vol3D_Decoder'):
		with argscope([Conv3D], kernel_shape=3, padding='SAME', use_bias=False,  nl=tf.nn.elu):
			with argscope([Conv3DTranspose], kernel_shape=3, padding='SAME', use_bias=False, nl=tf.nn.elu):
				...
				x = tf.transpose(x, [0, 2, 3, 1, 4], 'tp1')
				x = Conv3DTranspose('conv6a', x, 256, strides = 2) #
					# .Conv3D('conv6b',   	   256, strides = 1) #
					# .Conv3D('conv6c',   	   256, strides = 1) #
				x = tf.transpose(x, [0, 2, 3, 1, 4], 'tp2')
				x = Conv3DTranspose('conv5a', x, 128, strides = 2) #
					# .Conv3D('conv5b',   	   128, strides = 1) #
					# .Conv3D('conv5c',   	   128, strides = 1) #
				x =	tf.transpose(x, [0, 2, 3, 1, 4], 'tp3')
				x = Conv3DTranspose('conv4a', x,  64, strides = 2) #
					# .Conv3D('conv4b',   	    64, strides = 1) #
					# .Conv3D('conv4c',   	    64, strides = 1) #
				x = tf.transpose(x, [0, 2, 3, 1, 4], 'tp4')
				x = Conv3DTranspose('conv3a', x,  32, strides = 2) #
					# .Conv3D('conv3b',   	    32, strides = 1) #
					# .Conv3D('conv3c',   	    32, strides = 1) #
				x = tf.transpose(x, [0, 2, 3, 1, 4], 'tp5')
				x = Conv3DTranspose('conv2a', x,  16, strides = 2) #
					# .Conv3D('conv2b',   	    16, strides = 1) #
					# .Conv3D('conv2c',   	    16, strides = 1) #
				x =	tf.transpose(x, [0, 2, 3, 1, 4], 'tp6')
				x = Conv3DTranspose('conv1a', x,   1, strides = 2, use_bias=True, nl=tf.nn.tanh) #
...
class RenderingRunner(Callback):
	...
	def _trigger(self):
		...
		resultDir = 'result_stylish_3d_2d_vgg/'

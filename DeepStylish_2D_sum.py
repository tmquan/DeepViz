...
def normalize(v):
	...
	return v / tf.reduce_mean(v, axis=[0, 1, 2], keepdims=True)
###################################################################################################
	...
	def vgg19_encoder(self, inputs, name='VGG19_Encoder'):
		#with varreplace.freeze_variables():
			with argscope([Conv2D], kernel_shape=3, nl=BNLReLU):
				...
				conv1_1 = Conv2D('conv1_1', inputs,  8)
				conv1_2 = Conv2D('conv1_2', conv1_1, 8)
				pool1 = MaxPooling('pool1', conv1_2, 2)  # 64
				conv2_1 = Conv2D('conv2_1', pool1,   16)
				conv2_2 = Conv2D('conv2_2', conv2_1, 16)
				pool2 = MaxPooling('pool2', conv2_2, 2)  # 32
				conv3_1 = Conv2D('conv3_1', pool2,   32)
				conv3_2 = Conv2D('conv3_2', conv3_1, 32)
				#conv3_3 = Conv2D('conv3_3', conv3_2, 32)
				#conv3_4 = Conv2D('conv3_4', conv3_3, 32)
				pool3 = MaxPooling('pool3', conv3_2, 2)  # 16
				conv4_1 = Conv2D('conv4_1', pool3,   48)
				...
				return normalize(conv4_1), [normalize(conv1_1), normalize(conv2_1), normalize(conv3_1)]

	@auto_reuse_variable_scope
	def vgg19_decoder(self, inputs, feats, name='VGG19_Decoder'):
		with argscope([Conv2D], kernel_shape=3, nl=BNLReLU):	
			with argscope([Deconv2D], kernel_shape=3, strides=(2,2), nl=BNLReLU):
				pool3 = Subpix2D('pool3',   inputs,  32)  # 16
				#conv3_4 = Conv2D('conv3_4', pool3,   32)
				#conv3_3 = Conv2D('conv3_3', conv3_4, 32)
				conv3_2 = Conv2D('conv3_2', pool3, 32)
				conv3_1 = Conv2D('conv3_1', conv3_2, 32)+feats[2]
				pool2 = Subpix2D('pool2',   conv3_1, 16)  # 8
				conv2_2 = Conv2D('conv2_2', pool2, 	 16)
				conv2_1 = Conv2D('conv2_1', conv2_2, 16)+feats[1]
				pool1 = Subpix2D('pool1',   conv2_1, 8)  # 64
				conv1_2 = Conv2D('conv1_2', pool1, 	 8)
				conv1_1 = Conv2D('conv1_1', conv1_2, 8)+feats[0]
				...
				# conv3_4 = Conv2D('conv3_4', pool3,   128)
				# conv3_3 = Conv2D('conv3_3', conv3_4, 128)
				# conv3_2 = Conv2D('conv3_2', conv3_3, 128)
				# conv3_1 = Conv2D('conv3_1', conv3_2, 128)
				# pool2 = Subpix2D('pool2',   conv3_1, 128)  # 32
				# conv2_2 = Conv2D('conv2_2', pool2, 	 64)
				# conv2_1 = Conv2D('conv2_1', conv2_2, 64)
				# pool1 = Subpix2D('pool1',   conv2_1, 64)  # 64
				# conv1_2 = Conv2D('conv1_2', pool1, 	 32)
				# conv1_1 = Conv2D('conv1_1', conv1_2, 32)
				# conv1_0 = Conv2D('conv1_0', conv1_1, 3)
				...
				# pool3 = Subpix2D('pool3',   inputs,  32)  # 16
				# conv3_2 = Conv2D('conv3_2', pool3,   32)
				# pool2 = Subpix2D('pool2',   conv3_2, 16)  # 32
				# conv2_2 = Conv2D('conv2_2', pool2, 	 16)
				# pool1 = Subpix2D('pool1',   conv2_2, 8)  # 64
				# conv1_2 = Conv2D('conv1_2', pool1, 	 8)
				# conv1_0 = Conv2D('conv1_0', conv1_2, 3)
		...
		with tf.variable_scope('gen'):
			# with tf.device('/device:GPU:0'):
				...
				with tf.variable_scope('encoder_vgg19'):
					vol3d_encoded, vol3d_feature = self.vgg19_encoder(vol3d)
				...
				with tf.variable_scope('style_transfer'):
					...
					chose_encoded = tf.cond(condition > 0, # if istest turns on, perform statistical transfering
											...
											lambda: tf.identity(vol3d_encoded)) #else get the img2d_encoded
					merge_feature = [] #tf.zeros_like(vol3d_feature)
					chose_feature = [] #tf.zeros_like(vol3d_feature)
					for img2d_feat, vol3d_feat in zip(img2d_feature, vol3d_feature):
						merge_feat = self._build_adain_layers(vol3d_feat, img2d_feat)
						chose_feat = tf.cond(condition > 0, # if istest turns on, perform statistical transfering
											lambda: tf.identity(merge_feat), 
											lambda: tf.identity(vol3d_feat)) #else get the img2d_encoded
						merge_feature.append(merge_feat)
						chose_feature.append(chose_feat)
			...
			# with tf.device('/device:GPU:1'):
				...
				with tf.variable_scope('decoder_vgg19'):
					vol3d_decoded = self.vgg19_decoder(chose_encoded, chose_feature)
					img2d_decoded = self.vgg19_decoder(img2d_encoded, img2d_feature)
...
class ImageDataFlow(RNGDataFlow):
	def __init__(self, image_path, style_path, size, dtype='float16', isTrain=False, isValid=False, isTest=False):
		self.dtype      	= dtype
...
class RenderingRunner(Callback):
	...
	def _trigger(self):
		...
		resultDir = 'result_stylish_2d_sum/'
...
if __name__ == '__main__':
	...
	if args.render:
		...
		if args.load:
			...
			# print(weight)
			param_dict = {}
			param_dict.update({'gen/encoder_vgg19/' + name: value.astype(np.float32) for name, value in six.iteritems(weight)})
			session_init = None #DictRestore(param_dict)

...
class Model(ChainModelDesc):
	...
	def vol3d_encoder(self, x, name='Vol3D_Encoder'):
		...
		# 	x = tf_2tanh(x)
		# 	x = (LinearWrap(x)
		# 			.Conv2D('conv1', DIMZ/2, ) # 128
		# 			.Conv2D('conv2', DIMZ/4, ) # 64				
		# 			.Conv2D('conv3', DIMZ/8, ) # 32
		# 			.Conv2D('conv4', DIMZ/16,) # 16
		# 			.Conv2D('conv5', DIMZ/32,) # 8
		# 			.Conv2D('conv6', DIMZ/64,) # 4
		# 			.Conv2D('conv7', DIMZ/128) # 2
		# 			.Conv2D('conv8', DIMZ/256, use_bias=True, nl=tf.nn.tanh) # 1
		# 			())
	...
	def vol3d_decoder(self, x, name='Vol3D_Decoder'):
		...
		# 		x = (LinearWrap(x)
		# 			.Conv3DTranspose('conv6a', 256, strides = 2) #
		# 			# .Conv3D('conv6b',   	   256, strides = 1) #
		# 			# .Conv3D('conv6c',   	   256, strides = 1) #
		# 			.Conv3DTranspose('conv5a', 128, strides = 2) #
		# 			# .Conv3D('conv5b',   	   128, strides = 1) #
		# 			# .Conv3D('conv5c',   	   128, strides = 1) #
		# 			.Conv3DTranspose('conv4a',  64, strides = 2) #
		# 			# .Conv3D('conv4b',   	    64, strides = 1) #
		# 			# .Conv3D('conv4c',   	    64, strides = 1) #
		# 			.Conv3DTranspose('conv3a',  32, strides = 2) #
		# 			# .Conv3D('conv3b',   	    32, strides = 1) #
		# 			# .Conv3D('conv3c',   	    32, strides = 1) #
		# 			.Conv3DTranspose('conv2a',  16, strides = 2) #
		# 			# .Conv3D('conv2b',   	    16, strides = 1) #
		# 			# .Conv3D('conv2c',   	    16, strides = 1) #
		# 			.Conv3DTranspose('conv1a',   1, strides = 2, use_bias=True, nl=tf.nn.tanh) #
		# 			# .Conv3D('conv1b',   	     4, strides = 1) #
		# 			# .Conv3D('conv1c',   	     1, strides = 1, use_bias=True) #
		# 			()) 
		# 		x = tf.transpose(x, [4, 1, 2, 3, 0]) # 
		# 		x = tf.squeeze(x)
		# 		# x = x*255.0
	...
	def _build_graph(self, inputs):
		...
		self.scope_1_loss =  tf.reduce_sum([1e1*loss_vol3d, 1e0*loss_img3d], name='scope_1_loss')
		...
		with tf.name_scope('visualization'):
			...
			viz = tf.concat([tf.concat([viz_vol_1, viz_vol_2, viz_vol_3, viz_vol_8, viz_img_4], 2), 
							 tf.concat([viz_vol_5, viz_vol_6, viz_vol_7, viz_vol_9, viz_zeros], 2), 
...
if __name__ == '__main__':
	...
	if args.render:
		...
		if args.load:
			...
			param_dict = {}
			param_dict.update({'scope_2/encoder_vgg19/' + name: value for name, value in six.iteritems(weight)})

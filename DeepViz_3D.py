...
VGG19_MEAN_TENSOR = tf.constant(VGG19_MEAN, dtype=tf.float16)
###############################################################################
...
class Model(ModelDesc):
	...
	def _get_inputs(self):
		return [
			InputDesc(tf.float16, (DIMZ, DIMY, DIMX, 3), 'image'),
			InputDesc(tf.float16, (   1, DIMY, DIMX, 3), 'style'),
...
if __name__ == '__main__':
	...
	if args.render:
		...
		if args.load:
			...
			param_dict = {}
			param_dict.update({'gen/encoder_vgg19/' + name: value.astype(np.float16) for name, value in six.iteritems(weight)})
